...
static long xround (LDOUBLE value)
{
  long intpart;
		   ...
		   LDOUBLE fvalue, int min, int max, int flags)
{
   ...
   */
  fracpart = xround ((pow10 (max)) * (ufvalue - intpart));
...
void vsnprintf (OutputStream<char>& buffer, long maxlen, const char *fmt, va_list args)
{
  dopr(buffer, maxlen, fmt, args);
}

void snprintf (OutputStream<char>& buffer, long maxlen, const char *fmt,...)
{
  VA_LOCAL_DECL;
